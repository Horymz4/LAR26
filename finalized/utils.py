...
def reasoning(pos, radius, avg_x, height, ratio):
    ...
    # if we are facing the open side of a garage drive forwards
    if (not exited_garage.is_set()) and ratio is not None and ratio < 0.15:
        exited_garage.set()

    # if we are facing the ball drive towards it
    if (not garage_stage.is_set()) and radius is not None and \
            pos is not None and 150 > radius > 5 and \
            IMG_CENTER_X + THRESHOLD > pos[0] > IMG_CENTER_X - THRESHOLD:
        garage_stage.set()
        print("SEE BALL")

    # if we are close to the ball stop
    if (not outgarage_stage.is_set()) and radius is not None and \
            pos is not None and \
            IMG_CENTER_X + 100 > pos[0] > IMG_CENTER_X - 100 and \
            60 > radius > 54:
        outgarage_stage.set()
        print("BALL CLOSE")

    # if we see garage drive towards it
    if (not see_garage.is_set()) and avg_x is not None and \
            IMG_CENTER_X + THRESHOLD > avg_x > IMG_CENTER_X - THRESHOLD:
        see_garage.set()
        print("SEE GARAGE")

    # when we are close to the garage start parking
    if (not ending_stage.is_set()) and see_garage.is_set() and \
            avg_x is None and height is None:
        ending_stage.set()
        print("PARKING")
...
def ball_image(rgb, ref_img):
    ...
    if pos is not None:
        print("position: ", pos, " radius: ", radius)
    return pos, radius
...
def P_reg_ball():
    ...
    if radius is not None and pos is not None and \
            IMG_CENTER_X - 110 < pos < IMG_CENTER_X + 110 and radius < 55:
