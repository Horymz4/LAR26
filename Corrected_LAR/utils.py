from threading_variables import Button_press, StateofBumper, processing_image, garage_stage, outgarage_stage, see_garage, ending_stage, vision_data, vision_lock
...
def reasoning(pos,radius,avg_x,dist):
    if (not garage_stage.is_set()) and radius is not None and pos is not None and 150 > radius > 5  and IMG_CENTER_X + 30 > pos[0] > IMG_CENTER_X - 30:
        garage_stage.set()
        print("SEE BALL")
    if (not outgarage_stage.is_set()) and radius is not None and pos is not None and IMG_CENTER_X + 40 > pos[0] > IMG_CENTER_X -40 and 60 > radius > 55:
        outgarage_stage.set()
        print("BALL CLOSE")
...
def P_reg_ball():
    ...
    if radius is not None and pos is not None and IMG_CENTER_X + 30 > pos[0] > IMG_CENTER_X - 30 and radius < 55:
